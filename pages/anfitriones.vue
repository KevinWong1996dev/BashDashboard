<template>
  <div class="container-fluid">
    <main class="column main">
      ...
      <b-table
        :data="isEmpty ? [] : anfitriones_validados"
        :bordered="isBordered"
        :striped="isStriped"
        :narrowed="isNarrowed"
...
export default {
  ...
  computed: {
    ...mapState({
      anfitriones_validados: state => state.bash_master.anfitriones_validados
    })
  },

  methods: {
    ...mapActions({
      get_anfitriones_validados: 'bash_master/get_anfitriones_validados',
      // get_masters: 'bash_master/get_masters',
      rechazarmaster: 'bash_master/rechazarmaster'
    }),
    async eliminar(anfitriones_validados) {
      this.$swal({
        type: 'info',
        title: '¿Desea Rechazar el Anfitrión?',
        text: anfitriones_validados.username,
        confirmButtonText: 'Ok'
      })
        .then(async () => {
          await this.rechazarmaster(anfitriones_validados)
          await this.get_anfitriones_validados()
        })
        .catch(error => {
          this.$swal({
            type: 'error',
    ...
    //   })
    //     .then(async () => {
    //       await this.eliminarmaster(master)
    //       await this.get_masters()
    //     })
    //     .catch(error => {
    //       this.$swal({
    //         type: 'error',
    ...
    //   this.edituser = JSON.parse(JSON.stringify(anfitriones))
  ...
  created() {
    this.get_anfitriones_validados()
  },
